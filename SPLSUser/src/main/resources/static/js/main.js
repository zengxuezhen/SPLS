        ...
        success:function(result){
            if(result.success){
              if(result.value.code===null){
                if(result.value.body.indexOf("<form")===0){
                  var divElement = document.createElement("div");
                  divElement.innerHTML=result.value.body;
                ...
                self.setResult(JSON.stringify(result.value));
              }
            }else{
        ...
        for(var k in data){
            if(data[k].childs.length>0){
                ...
                $form.find('label').html(label).end()
                  ...
                  .find('.notice').html(data[k].description).end();
                result.push($form);
            }
        }
        return result
